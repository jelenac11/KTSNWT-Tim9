...
public interface IAdminRepository extends JpaRepository<Admin, Long> {
	...
	@Query(value ="SELECT * FROM users_table a WHERE a.type='AD' AND ((LOWER(a.username) LIKE ?1) OR (LOWER(a.email) LIKE ?1) OR (LOWER(a.first_name) LIKE ?1) OR (LOWER(a.last_name) LIKE ?1))", nativeQuery=true)
